...
def create_uniform_kernel(width, height):
    kernel = np.ones((height, width)) / 2#/ (width*height)
    return kernel


def create_gauss_kernel(width, height):
    width, height = width + abs(width%2-1), height + abs(height%2-1)
    x, y = np.meshgrid(np.linspace(-1, 1, width), np.linspace(-1, 1, height))
    kernel = np.exp(-(np.sqrt(x * x + y * y) ** 2 / (2.0 * 10 ** 2)))
    return kernel
